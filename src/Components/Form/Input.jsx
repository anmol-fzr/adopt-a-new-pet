...
export default function Input({ type = "text", label, value, onChange, placeholder, error, optional, desc, className }) {
    ...
    return (
        <div className='flex flex-col relative' >
            ...
            <input id={label} type={type} inputMode={inputMode} placeholder={placeholder} value={value} onChange={onChange} onFocus={() => setIsFocused(true)}
                className={`p-2  outline-none rounded border border-gray-600/50 hover:border-gray-300 ${error ? "border-red-500 focus:border-red-500 " : " focus:border-indigo-600  "} ${className} `} />
